...
public class Zolnierz extends Osoba {
    ...
    static final int dniowkaSzeregowego = 169, dniowkaPodoficera = 190, dniowkaOficera = 240;
    ...
    public int getZarobki(int liczbaDni) {
        return switch(this.korpus){
            case SZEREGOWY -> dniowkaSzeregowego*liczbaDni;
            case PODOFICER -> dniowkaPodoficera*liczbaDni;
            case OFICER -> dniowkaOficera*liczbaDni;
        };
    }
    ...
    public void removeFromExtent() {
        ...
        if(this.podwladni!=null && this.podwladni.size()>0){
            while(this.podwladni.size()>0){
                removePodwladny(this.podwladni.get(0));
            }
        }
